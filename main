...
local Window = Rayfield:CreateWindow({
	...
	KeySettings = {
		...
		Key = {"https://pastebin.com/raw/QigdVziG"} 
	}
})
...
local TeleportSection = TeleportTab:CreateSection("Pick a destination where you want to go")
local placeId = game.PlaceId
-- city id: 3101667897, space id: 3232996272 (Spawn: CFrame.new(277.64, 72.95, -266.66, -1.00, -0.00, 0.04, -0.00, 1.00, -0.00, -0.04, -0.00, -1.00), to earth: CFrame.new(338.39, 72.95, -285.04, 0.52, 0.00, -0.85, -0.00, 1.00, 0.00, 0.85, -0.00, 0.52)), desert id: 3276265788 (Spawn: CFrame.new(255.68, 645.84, -205.19, -0.01, -0.00, 1.00, 0.00, 1.00, 0.00, -1.00, 0.00, -0.01), to earth: CFrame.new(286.14, 645.84, -233.03, 0.59, 0.00, -0.81, 0.00, 1.00, 0.00, 0.81, -0.00, 0.59) )

local Toggle10000 = TeleportTab:CreateToggle({
	Name = "Fast teleport",
	Default = false,
	Callback = function(Value)
...
local Button = TeleportTab:CreateButton({
 ...
 local Button = TeleportTab:CreateButton({
 	...
 	Callback = function()
 		...
 		elseif placeId == 3232996272 then -- Space
 			game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(277.64, 72.95, -266.66, -1.00, -0.00, 0.04, -0.00, 1.00, -0.00, -0.04, -0.00, -1.00)
 		elseif placeId == 3276265788 then -- Desert
 			game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(286.14, 645.84, -233.03, 0.59, 0.00, -0.81, 0.00, 1.00, 0.00, 0.81, -0.00, 0.59)
 		end
 	end    
...
local Button = TeleportTab:CreateButton({
	...
	Callback = function()
		...
		elseif placeId == 3232996272 or placeId == 3276265788 then -- Space or Desert
			game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(338.39, 72.95, -285.04, 0.52, 0.00, -0.85, -0.00, 1.00, 0.00, 0.85, -0.00, 0.52)
		end
	end    
})
...
local TeleportSection = TeleportTab:CreateSection("Outer outer space Crystals Location")
...
local Button = TeleportTab:CreateButton({
	...
	Callback = function()
		...
		elseif placeId == 3101667897 then -- Main place
			game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-334.00, 3.75, 585.00, -0.63, -0.00, -0.78, -0.00, 1.00, -0.00, 0.78, 0.00, -0.63)
		elseif placeId == 3276265788 then -- Desert
			game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(286.14, 645.84, -233.03, 0.59, 0.00, -0.81, 0.00, 1.00, 0.00, 0.81, -0.00, 0.59)
		end
	end    
...
local Button = TeleportTab:CreateButton({
	...
	Callback = function()
		...
		elseif placeId == 3101667897 then -- Main place
			game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(2516.00, 14.64, 4356.00, -0.09, -0.00, -1.00, -0.00, 1.00, -0.00, 1.00, 0.00, -0.09)
		elseif placeId == 3232996272 then -- Space
			game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(338.39, 72.95, -285.04, 0.52, 0.00, -0.85, -0.00, 1.00, 0.00, 0.85, -0.00, 0.52)
		end
	end    
		...
		if Value then
			...
			spawn(function()
				while Value do
					...
					wait(0.01)  -- Check every millisecond
				end
			end)
		end
	end,
...
local AutoBuySection = AutoBuyTab:CreateSection("Select pet (uses gems)")
...
local AutoAcceptOfferToggle = AutoTradeTab:CreateToggle({
 ...
 local Section = AutoTradeTab:CreateSection("Enable/Disable trades (game has buttons for it💀)")
...
local Toggle = AutoRaceTab:CreateToggle({
	...
	Callback = function(state)
		if state then 
			_G.loop = true 
			while _G.loop == true do wait(0.05) 
				--local args = {
				--	[1] = "joinRace"
				--}
				...
				--local args = {
				--	[1] = "firstPlace",
				--	[2] = game.Players.Fire_kreeper,
				--	[3] = 500
				--}
				--game.ReplicatedStorage.rEvents.raceEvent:FireServer(unpack(args))
				--task.wait(15)
				local args = {
					[1] = "joinRace"
				}
...
local AutoRaceToggle = AutoRaceTab:CreateToggle({
	...
	Callback = function(state)
		if state then
			...
			spawn(function()
				local savedPosition = nil  -- Track the saved position only once per race
				while _G.autoRaceLoop do
					...
					-- Wait for the race timer to hit 0
					while raceTimer.Value > 0 and _G.autoRaceLoop do
						task.wait(0.1)
					end

					-- Save position only once when the race starts
					if character and character:FindFirstChild("HumanoidRootPart") and not savedPosition then
						...
						if map then
							...
							if teleportCFrame then
								...
								while _G.autoRaceLoop do
									...
									if character and character:FindFirstChild("HumanoidRootPart") then
										-- If the player's position has changed from the saved position, teleport them back
										if character.HumanoidRootPart.CFrame ~= savedPosition and not hasTeleported then
					...
					-- Reset saved position if the timer resets (when race starts again)
					if raceTimer.Value == 60 then  -- Assuming raceTimer resets back to 60 seconds
						savedPosition = nil  -- Reset saved position for the next race
					end
				end
			end)
		else
...
local CharacterTab = Window:CreateTab("Characters", 4483362458)


local GlitchableTab = Window:CreateTab("Rebirths", 4483362458)
...
--local Button = MiscTab:CreateButton({
--	Name = "Snow Queen Torso",
--	Callback = function()
--		local player = Players.LocalPlayer

--		-- Create a new HumanoidDescription
--		local newDescription = Instance.new("HumanoidDescription")

--		-- Set the torso asset ID
--		newDescription.Torso = 187996321

--		-- Prompt the player to save the avatar change
--		AvatarEditorService:PromptSaveAvatar(newDescription, Enum.HumanoidRigType.R15)

--		-- Wait for the prompt to complete
--		local result = AvatarEditorService.PromptSaveAvatarCompleted:Wait()

--		if result == Enum.AvatarPromptResult.Success then
--			print("Snow Queen torso equip attempt successful")

--			-- Check if the torso actually changed
--			task.wait(1) -- Wait a short time for the change to apply
--			local character = player.Character
--			if character and character:FindFirstChild("UpperTorso") then
--				local upperTorso = character.UpperTorso
--				print("Current UpperTorso MeshId:", upperTorso.MeshId)
--			else
--				print("Could not find UpperTorso to verify change")
--			end
--		else
--			warn("Failed to equip Snow Queen torso")
--		end
--	end,
--})
